---
description: 发布新版本：升级版本号 + 整理上次发版到当前的全部提交 + 写入 Dsh/version.json 更新清单 + 提交并推送到远端。push 后 GitHub 工作流会自动据此生成 Release 说明与安装包。
...
### 4. 生成中文更新清单并写入 Dsh/version.json
...
- 把这段清单写入仓库内 `Dsh/version.json` 的 `notes` 字段（保留原有 `version`/`url` 字段，仅更新 `notes`）。直接用 PowerShell 读取现有 JSON 对象、更新 notes 后再以 UTF-8 无 BOM 写回，避免硬编码字段名或破坏缩进：
  ```powershell
  $p = "Dsh/version.json"
  $vj = Get-Content $p -Raw | ConvertFrom-Json
